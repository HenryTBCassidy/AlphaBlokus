...
def test_action_ids_unique_and_in_range(tables) -> None:
    ...
    assert len(np.unique(tables.action_id)) == tables.num_moves, "Duplicate action_id in the tables — encoding bug."
...
def test_cells_padded_with_null(tables) -> None:
    ...
    for i in range(tables.num_moves):
        ...
        assert (real_cells < NUM_CELLS).all(), f"Move {i}: cell index >= NUM_CELLS in real-cell region"
        assert (padded == NULL_CELL).all(), f"Move {i}: cells beyond n_cells={n} not padded with NULL_CELL"
...
def test_no_duplicate_cells_per_move(tables) -> None:
    ...
    for i in range(tables.num_moves):
        ...
        assert len(set(cells.tolist())) == n, f"Move {i} has duplicate footprint cells: {cells.tolist()}"
...
def test_adj_and_attach_disjoint_from_footprint(tables) -> None:
    ...
    for i in range(tables.num_moves):
        ...
        assert footprint.isdisjoint(adj), f"Move {i}: footprint overlaps with adj cells"
        assert footprint.isdisjoint(attach), f"Move {i}: footprint overlaps with attach cells"
...
def test_adj_and_attach_disjoint_from_each_other(tables) -> None:
    ...
    for i in range(tables.num_moves):
        ...
        attach = set(tables.attach_cells[i, : tables.n_attach[i]].tolist())
        overlap = adj & attach
        assert not overlap, f"Move {i}: cell(s) {overlap} in both adj and attach sets"
...
def test_action_to_move_round_trip(tables) -> None:
    ...
    for move_id in range(tables.num_moves):
        ...
        assert round_tripped == move_id, (
            f"Round-trip broken for move_id {move_id}: action {action_id} → move {round_tripped} (expected {move_id})"
        )
...
def test_offboard_actions_marked(tables) -> None:
    ...
    assert legal_count == tables.num_moves, f"Expected {tables.num_moves} legal action slots, got {legal_count}"
...
def test_lookup_slices_dont_overflow(lookup) -> None:
    ...
    assert (end <= lookup.total_entries).all(), "Some slot's begin+size exceeds move_ids buffer length"
...
def test_adj_status_cells_correct_for_anchor_middle(lookup) -> None:
    ...
    for bit, (dr, dc) in enumerate(ADJ_STATUS_OFFSETS):
        ...
        assert actual == expected_cell, f"adj_status_cells[(7,7), bit {bit}] = {actual}, expected {expected_cell}"
...
def test_candidate_lists_have_correct_move_ids(tables, lookup) -> None:
    ...
    for anchor, status, piece_idx in zip(sample_anchors, sample_statuses, sample_pieces, strict=True):
        ...
        for move_id in lookup.move_ids[begin : begin + sz]:
            ...
            assert not overlap, f"Move {mv} at ({anchor},{status},{piece_idx}) occupies forbidden cells {overlap}"
