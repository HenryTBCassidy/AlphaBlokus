...
from core.coach import Coach  # noqa: E402
...
from games.blokusduo.neuralnets.wrapper import NNetWrapper  # noqa: E402


@pytest.mark.slow
def test_one_generation_blokus_jax_backend(tmp_path: Path) -> None:
    config = RunConfig(
        game="blokusduo", run_name="test_jax_loop", num_generations=1, num_eps=2,
        temp_threshold=12, update_threshold=0.55, num_arena_matches=2,
        root_directory=tmp_path, load_model=False,
        mcts_config=MCTSConfig(
            num_mcts_sims=8, cpuct=2.5, dirichlet_epsilon=0.25, dirichlet_alpha=0.03,
        ),
