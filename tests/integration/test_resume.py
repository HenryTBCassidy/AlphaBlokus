...
def test_resume_continues_without_clobber(
    ...
    """
    # The gen-0 anchor checkpoint is always created now (retained for the pool
    # tournament) and must be preserved on resume; a small games-sized buffer
    # (num_eps=2 → 2 games/gen) forces the resume reload to span multiple
    # generation files.
    cfg = replace(
        test_config,
        num_generations=2,
