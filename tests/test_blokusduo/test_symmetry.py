...
# ── S7: round-trip on randomly-seeded boards ────────────────────────────────


def test_board_transpose_involution_on_random_rollouts(
    blokus_game: BlokusDuoGame,
) -> None:
    """T7 — `transposed().transposed()` returns to the placement state on
    boards generated by random legal-move rollouts of varying length.

    The hand-built fixtures in S2 cover specific cases; this adds breadth
    ...
    for rollout in range(5):
        ...
        player = 1
        # Vary rollout length to hit early / mid / later game states.
        rollout_length = rng.integers(2, 16)
