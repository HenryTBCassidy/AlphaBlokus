...
23. **Every stored self-play position records the side to move, and pre-D1 files are refused (2026-08-18).** `ProcessedExample` is a `NamedTuple` — `(board, policy, value, player)`, `player` = `+1` White / `-1` Black — persisted as an int8 parquet column behind a `player_kind` marker. **Never re-derive the colour from the board.** The stored board is canonical, and `infer_mover_colour`'s piece-count parity only works until the first pass, which is an endgame event — exactly where the value signal matters. A self-play parquet written before this **fails to load** rather than being read colour-blind (same refuse-don't-guess rule as `board_kind`/`policy_kind`); resume such runs from their checkpoints. `train()` asserts every position carries `±1` across the whole buffer. The **dataset item shape stays `(board, pi, value)`** plus one tensor per built auxiliary head — a loss term that wants the colour reads `example.player` and passes it through the aux-target-source seam, because `_AuxTargetDataset` and the training loop's `zip(aux_names, extra, strict=True)` both depend on that layout. Plan: `docs/plans/selfplay-data-and-loop.md` D1.
