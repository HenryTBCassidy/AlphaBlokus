...
11. **Weight decay is on by default for every run (2026-07-22).** The optimizer is `AdamW` with `NetConfig.weight_decay = 1e-4` (`games/base_wrapper.py`); this deliberately changes all existing configs' behaviour — training without it let `blokus_paired_gate_rerun` drift a converged net L4 → L3 (`docs/research/regression-and-next-steps.md` §1.3). `load_checkpoint` re-asserts the configured decay so pre-change checkpoints can't silently disable it on `--resume`. Set `"weight_decay": 0.0` only to reproduce an old run bit-for-bit. Warm-start *continuations* should also run `epochs: 1` at LR ~2.5e-4 (plan P2).
