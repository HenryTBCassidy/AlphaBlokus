...
def default_pieces_path() -> Path:
    """Filesystem path of the canonical ``pieces.json`` shipped with the package.

    Single source of truth for locating the piece definitions — callers must
    not construct their own repo- or CWD-relative paths to the file.
    """
    return Path(str(files("alphablokus.games.blokusduo").joinpath("pieces.json")))
