...
def _fill_mask_kernel(
    forbidden: NDArray[np.uint8],
    anchors: NDArray[np.int32],
    adj_status_cells: NDArray[np.uint8],
    lookup_begin: NDArray[np.uint32],
    lookup_size: NDArray[np.uint16],
    lookup_move_ids: NDArray[np.uint16],
    move_cells: NDArray[np.uint8],
    move_action_id: NDArray[np.uint32],
    remaining: NDArray[np.int32],
    out: NDArray[np.bool_],
    seen: NDArray[np.bool_],
) -> None:
    """Fill ``out`` (bool action mask) with every legal move across ``anchors``.

    The annotations document the dtypes Numba specialises on (its type inference
    works off the runtime arrays, not these hints — they are for humans/linters).
    Marshalling that passes a different dtype would change the compiled
    specialisation, so keep them accurate.
...
_DEFAULT_PIECES_PATH = default_pieces_path()
