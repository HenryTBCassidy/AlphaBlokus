...
| [`deepmind-run-configs.md`](deepmind-run-configs.md) | Grounded reference for the run-config numbers DeepMind actually used across AlphaGo → AlphaGo Zero → AlphaZero → MuZero — hardware, total games, generation structure, MCTS sims, net size, replay window, wall-clock — each mapped to what it means for our `RunConfig` choices. The reference we used to design `blokus_run2_bignet`. Cited to the primary papers. |
| [`jax-spike-findings.md`](jax-spike-findings.md) | Results of the JAX de-risk spike (`docs/plans/archive/jax-spike.md`): Blokus Duo rules as three int8 matmuls in JAX — exact parity with the Python engine everywhere, env at 570k steps/s on the 3060 Ti (54× the whole production pipeline), self-play becomes purely inference-bound at ~3× production sims/s on this card. Verdict vs the pre-registered criteria + the conditional-go path (Gumbel low-sim search + 5070 Ti → 10–50×). |
| [`jax-pipeline-ab.md`](jax-pipeline-ab.md) | G7/G8 results for the JAX self-play pipeline: where search wall-time actually goes (mctx tree machinery ∝ top_k), backend throughput (PUCT ≈ parity; Gumbel n=64 = 13.5 games/s at 64f×4b, ~12× at 128f×8b), and the three-arm A/B (jax-PUCT beats python head-to-head 60.5%; gumbel parity at 3.5× wall-clock). The flip decision and its gate. |
