...
- Read with `iter_game_examples()` / `iter_opening_examples()` (both yield the pipeline's `(board, (indices, values), value)` tuples, with an optional load-time target temperature τ); validate with `validate_game_shard()` (full replay; asserts the target sums to 1, `support ⊆ legal` — never equality, since Pentobi searches 315 of 414 first moves — `action ∈ support`, and `top_action == argmax`) and `validate_opening_shard()` (structural, plus a witness-path replay when handed the store).
