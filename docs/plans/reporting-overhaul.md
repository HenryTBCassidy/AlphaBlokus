...
| R15 | Add `scripts/arena_run.py` CLI: `--player1 <ckpt\|random\|minimax> --player2 <...> --num-games <N> --game <ttt\|blokusduo>` — generic head-to-head runner using R12's Player abstraction. Used internally by R13/R14 and as a user-facing tool for ad-hoc evaluation. | Elo + scripts | 1 hr | Medium | |
| R16 | Add `scripts/play_ttt.py --checkpoint <path>`: human-vs-model TTT CLI. Renders 3×3 board, reads moves 0-8 from stdin, model plays back via MCTS with temp=0. Trivial once R12's `HumanPlayer` + `NetworkPlayer` exist. | Elo + scripts | 30 min | Medium | |
