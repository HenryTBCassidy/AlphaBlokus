...
| S5 | **Validation gate** — re-run the S3 known-gap + null controls through the *paired* instrument, colour split reported | ~1 h run | High | (box run + analysis) | ✅ (controls re-run through the paired instrument; gate cleared → the S7 rerun launched & ran to completion — outcome in [`../../research/regression-and-next-steps.md`](../../research/regression-and-next-steps.md)) |
