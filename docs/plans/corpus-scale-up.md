...
> **ACTIVE 2026-08-12 — scope owner `corpus`.** Moved out of `future/` because this plan carries the
> project's only *measured* claim about priority: the data-fraction curve showed **we are data-limited,
> decisively** — doubling the corpus buys 0.18–0.35 nats of held-out CE with no sign of flattening, and
> the conclusion was that **"generate more games" outranks all technique work**.
>
> 9 of 16 rows are done. The remaining work is long and low-attention — a ~3-day generation (V12), then
> the refit and the gate (V14/V15) — which makes it the natural background stream while shorter work
> runs alongside. **V11 is now [`evaluation-instruments.md`](evaluation-instruments.md) I3**; do not
> run it twice.
