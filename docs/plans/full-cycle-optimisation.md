...
F1–F4 are all **done** — ~14× cumulative wall-clock vs serial (M1 and M2 smashed), plus a 21.6×-smaller net from F4 (see [Progress tracker](#progress-tracker)). **No active step remains:** the one untapped speedup lever (a cross-worker inference server) is deferred, and a few other ideas were considered and **set aside** — see [Considered and set aside](#considered-and-set-aside). The plan stays open as the index/tracker; revisit Option B only if a future need pushes for M3.
