...
| F4 — Convolutional policy head | | | | | _Next — sub-plan [`conv-policy-head.md`](conv-policy-head.md). (MCTS tree reuse, the former F4 candidate, was set aside as already-implicit — see [Considered and set aside](#considered-and-set-aside).)_ |
