...
|---|---|---|---|
| Save densifies whole generation | `coach.py:938-942`, `storage.py:1120-1148` | CRITICAL | O1, O3 |
| Load rehydrates whole file dense (`to_pandas`/`iterrows`) | `storage.py:1168-1195` | CRITICAL | O2 |
| Resume rehydrates whole buffer dense (live=sparse, resume=dense) | `storage.py:1261-1300` | CRITICAL | O2 |
| JAX harvester holds dense 17,837 policy per open position | `jaxplay/harvest.py:49,98-103` | MODERATE | O4 |
| No JAX VRAM fraction cap on shared 8 GB card | `jaxplay/__init__.py`, `jaxenv/__init__.py` | MODERATE | O5 |
| Whole generation accumulated in host RAM before buffer | `parallel_self_play.py:504-506`, `coach.py:479-483,508-512`, `jaxplay/backend.py:141-167` | MODERATE | O6 |
| Report loads entire ArenaReplays history then samples 16×6 | `reporting/training.py:1595-1598`; `scripts/replay.py:73` | MODERATE | O7 |
| Repeated OOMs undetected until crash | — | process gap | O8 |
| Eval-set dense (safe at 200, scales badly); MCTS transient churn | `base_wrapper.py:312`, `mcts.py:242,262-275` | MINOR | O9 |
...
**Current state.** `Coach.save_self_play_history` (`core/coach.py:938-942`) turns the generation's in-RAM **sparse** policies into **dense** ones purely to persist them:
...
`SelfPlayStore.save` (`core/storage.py:1120-1126`) then serializes each dense policy to bytes (`p.tobytes()`) in a single DataFrame. The dense deque alone is ~25 GB at 10k games; the `tobytes` list and `from_pandas` copy stack more on top. This is the exact allocation that OOM-killed the box.

**Fix.** Never densify to persist. The live buffer policy is already `(indices, values)` (see `core/sparse_policy.py`). Persist those directly:
- `save_self_play_history`: pass the sparse examples straight through — delete the `as_dense` densify.
...
**Migration.** Mirror the existing legacy-refusal (`storage.py:1174`): a file missing `policy_kind` (i.e. old dense format) is refused with a clear message. No auto-migration — self-play parquets are per-run artifacts and we're starting fresh scaled runs.

---

## O2. Sparse read + resume path

**Current state.** `SelfPlayStore.load` (`storage.py:1168-1195`) reads the whole file, calls `table.to_pandas()`, then `for _, row in df.iterrows()` reshaping each policy to dense 17,837 — three full copies (Arrow + pandas + deque) and `iterrows` is pathologically slow at 800k rows. `load_recent_games` (`storage.py:1261-1300`) loops this newest-first up to `replay_buffer_games`, so **resume rebuilds the entire buffer dense** — ~125 GB at 50k games. This is a latent landmine: a run that trained fine (sparse live buffer) OOMs the instant it is `--resume`d (dense reload).
...
- Delete the dead `load_window` (`storage.py:1197`, not called anywhere) rather than leave the same dense pattern lying around.
...
**Current state.** Even with sparse policies, `save` builds the whole generation as one `pd.DataFrame` → `pa.Table.from_pandas` (`storage.py:1122-1148`) before writing — ~1.5–2 GB transient at 10k games.
...
**Current state.** `core/jaxplay/harvest.py:98-103` scatters each position's top-K trace into a fresh dense `np.zeros(17837)` and appends it to the open game's `slot.policies` (`harvest.py:49`), holding it dense until the game ends and only sparsifying in `_finish_game`. That's ~760 MB resident at `batch_size=256` and one `np.zeros(17837)` per ply×game (8,192/wave) of alloc/GC churn — a partial regression of the sparse-policy fix.
...
**Current state.** `XLA_PYTHON_CLIENT_PREALLOCATE=false` is correctly set (`core/jaxplay/__init__.py:26`, `games/blokusduo/jaxenv/__init__.py:21`), but there is no `XLA_PYTHON_CLIENT_MEM_FRACTION` cap. JAX growing on demand and fragmenting against torch's caching allocator on one 8 GB card is a classic mixed-run OOM (and directly relevant to any future inference-GPU / training-GPU split).
...
**Current state.** Self-play collects the **whole generation** in the main process before it reaches the buffer: `run_self_play_episodes_parallel` appends every game to `per_episode_examples` (`core/parallel_self_play.py:504-506`); the JAX backend builds `records` then `examples = [record.examples for ...]` (`core/jaxplay/backend.py:141,153,167`); the Coach then re-lists them (`coach.py:479-483,508-512`) and calls `replay_buffer.extend`. That's a ~1.5–2.5 GB sparse whole-generation copy coexisting with the buffer. Sparse, so not the trigger — but a real multi-GB spike that compounds with everything else.
...
**Current state.** `reporting/training.py:1595-1598` loads the **entire** ArenaReplays history (`_load_metrics` → `pd.read_parquet` over the whole dir, `training.py:25`) then renders only `_REPLAY_MAX_GENERATIONS=16 × _REPLAY_MAX_GAMES_PER_GEN=6` games — ~430 MB–1 GB discarded. `scripts/replay.py:73` reads the whole dir to print one game. (The report runs once end-of-run and is exception-wrapped, so this is not a run-killer — but it grows unbounded with generations.)
...
2. **Peak-RSS logging at phase transitions** — extend the existing `_get_memory_snapshot` (`coach.py:270`) to log peak RSS after self-play, after save, and after train, to console + W&B. Makes the next spike visible in the run, not post-mortem.
...
- **Eval set** (`core/coach.py:178`, `base_wrapper.py:312`): add an assert/comment pinning `_eval_set_size` — dense `(n, 17837)` is fine at 200 (~14 MB) but would OOM if scaled to the buffer. Optionally store the eval set sparse too.
- **MCTS** (`core/mcts.py:275`): `root_visit_counts` as `int32` (halves the per-move 143 KB); the per-move `.tolist()` / list-comprehensions at `:242,262-264` build full-action-space Python lists (~0.5 MB/move, ~99% zeros) — optional to trim, transient and bounded.
- **`predict_encoded`** (`base_wrapper.py:407`): full-policy GPU→CPU copy is bounded by batch (~18 MB) — leave as-is, noted for completeness.

---

## Confirmed safe (checked, no action)

- **Training loop** — `_LazyPolicyDataset` + `train()` (`base_wrapper.py:55-262`): compact boards, densified one DataLoader batch at a time. This is the pattern O1–O3 make the disk path mirror.
- **MCTS tree** — sparse per-node arrays sized to legal moves, never dense 17,837 (`mcts.py:76-96,476-509`).
- **Inference channel/server** — fixed-size shared buffers `(N,K,…)`, allocated once (`inference_channel.py:75-82`, `inference_server.py:129-193`).
...
- **Per-game self-play** — dense `pi` (incl. symmetry) held for one in-flight game only, sparsified at game end (`self_play.py:65-116`).
