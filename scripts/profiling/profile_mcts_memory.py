...
def main() -> None:
    ...
    print(
        f"profiling MCTS memory: game={config.game} sims={sims} f2={f2} "
    ...
    _write_html(
        config, sims, moves, n_states, _mb(unique_total), _mb(peak_bytes), bytes_per_state, bd_lines, top_lines, curve
    )

    print(f"\n  game: {moves} moves, {n_states} states, {n_state_actions} state-actions ({wall:.1f}s)")
    print(
        f"  TREE PEAK (unique): {_mb(peak_bytes):.1f} MB   end: {_mb(unique_total):.1f} MB"
...
def _write_md(
    ...
    L.append(
        "Generated by `scripts/profiling/profile_mcts_memory.py` "
    ...
    L.append(
        f"**Settings:** game `{config.game}` · {sims} sims · "
        f"net {config.net_config.num_filters}f×{config.net_config.num_residual_blocks}b · "
        f"action space {action_size} · F2 movegen {f2} · seed 42.\n"
    )
    L.append("## Baseline (the number to beat)\n")
    L.append(
        f"- **Peak tree memory (per worker): {peak_mb:.1f} MB** — this is what gates "
    ...
    L.append(
        "> Treat the peak MB/worker as immutable: every later change re-runs this profile "
    ...
    L.append(
        "> numpy array data buffers live outside Python's allocator, so tracemalloc can't "
    ...
    top_dict = bd_lines[0]
    L.append(
        f"- Biggest consumer: **`{top_dict[0]}`** at {top_dict[2]:.1f} MB "
        f"({top_dict[4]:.1f}% of the tree, {top_dict[3] / 1024:.1f} KB/entry)."
    )
    for n, info in value_info.items():
        L.append(f"- `{n}` stores: {info}.")
    L.append(
        "- See `docs/plans/mcts-memory-reduction.md` M3 for the candidate cuts; "
...
def _write_html(config, sims, moves, n_states, total_mb, peak_mb, bps, bd_lines, top_lines, curve) -> None:
    ...
    fig.update_layout(
        title="MCTS tree memory growth",
        xaxis_title="move",
        yaxis_title="tree MB (unique)",
    ...
    bd_rows = [
        (f"<code>{n}</code>", f"{cnt:,}", f"{mb:.1f} MB", f"{per / 1024:.2f} KB", f"{pct:.1f}%")
